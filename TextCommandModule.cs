...
namespace QobuzDiscordBot
{
    public class TextCommandModule : CommandModule<CommandContext>
    {
        ...
        public TextCommandModule(DataContext context) { _context = context; }
        ...
        [Command(["p", "play"])]
        public static async Task<string> Play(CommandContext context, [CommandParameter(Remainder = true)] string query)
        {
        ...
        public static async Task Kick(CommandContext context)
        {
            var client = context.Client;
            var guild = context.Guild;
        ...
        //todo fix root path:
        public static string? GetTrackPathById(string fileId) => Directory.EnumerateFiles(Path.Combine(/*_rootPath*/"./", "Music"), "*", SearchOption.AllDirectories).FirstOrDefault(f => Path.GetFileName(f).Contains(fileId)) ?? null;
    }
}
